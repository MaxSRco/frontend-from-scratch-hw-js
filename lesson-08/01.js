/*
  ...
  🧙 Подсказка: если пользователь кликнет по кнопке с питомцем, id питомца можно будет получить из объекта события (event.target.id)
*/
...
function addToCartList(event) {
    if (cart.length >= 3) {
        ...
        petShop.removeEventListener('click', addToCartList);
    } else {
...
petShop.addEventListener('click', addToCartList);
